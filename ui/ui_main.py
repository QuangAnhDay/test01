...
# Import từ các module đã tách theo cấu trúc mới
...
class PhotoboothApp(QMainWindow):
    ...
    def create_price_select_screen(self):
        ...
        for name, cfg in all_layouts.items():
            ...
            price = get_price_by_layout(name)
    ...
    def load_templates(self):
        ...
        # 1. Thư mục cụ thể cho layout (ví dụ: templates/4_2x2/ hoặc templates/2_1x2/)
        layout_folder = f"{self.selected_frame_count}_{self.layout_type}"
        
        # Danh sách các thư mục tiềm năng để tìm template
        search_dirs = [
            os.path.join(TEMPLATE_DIR, layout_folder),
            os.path.join(TEMPLATE_DIR, self.layout_type),
            os.path.join(TEMPLATE_DIR, self.layout_type.lower()),
        ]
        
        # Hỗ trợ thêm cho các layout custom như Custom_Custom1 -> tìm folder custom1
        if "Custom_" in self.layout_type:
            folder_part = self.layout_type.replace("Custom_", "").lower()
            search_dirs.append(os.path.join(TEMPLATE_DIR, folder_part))
            
        print(f"DEBUG: Selected Layout: {self.layout_type}, Photos: {self.selected_frame_count}")
        print(f"DEBUG: Searching templates in: {search_dirs}")

        for directory in search_dirs:
            if os.path.exists(directory):
                print(f"DEBUG: Found directory: {directory}")
        ...
        if os.path.exists(custom_dir):
            for f in os.listdir(custom_dir):
                ...
                if f.lower().endswith(('.png', '.jpg', '.jpeg')) and self.layout_type.lower() in f.lower():
                    templates.append(os.path.abspath(os.path.join(custom_dir, f)))

        # 3. Loại bỏ việc lấy từ thư mục gốc templates/ để đảm bảo ngăn nắp theo yêu cầu
        pass
        
        # Loại bỏ các đường dẫn trùng lặp và làm sạch đường dẫn
        templates = list(dict.fromkeys(templates))
        print(f"DEBUG: Found {len(templates)} templates for {self.layout_type}")
        for t in templates:
